...
template <typename FunctorType, unsigned rank, typename... Properties>
class ParallelFor<FunctorType,
                  Kokkos::TeamPolicy<Kokkos::Rank<rank>, Properties...>, HIP> {
 public:
  using Policy       = TeamPolicy<Kokkos::Rank<rank>, Properties...>;
  using functor_type = FunctorType;
  using size_type    = HIP::size_type;
 ...
 private:
  ...
  Policy const m_policy;
  size_type const m_league_size;
  Kokkos::Array<int, rank> const m_league_sizes;
  int m_team_size;
  size_type const m_vector_size;
  int m_shmem_begin;
  int m_shmem_size;
 ...
 public:
  ...
  __device__ inline Kokkos::Array<int, rank> get_indices(
      size_t league_rank) const {
    Kokkos::Array<int, rank> indices = {};
    for (int i = 0; i < rank; i++) {
      indices[i] = league_rank % m_league_sizes[i];
      league_rank /= m_league_sizes[i];
    }
    return indices;
  }
  ...
  __device__ inline void operator()() const {
    ...
    for (int league_rank = blockIdx.x; league_rank < int_league_size;
         league_rank += gridDim.x) {
      this->template exec_team<work_tag>(typename Policy::member_type(
          get_indices(league_rank), m_league_sizes,
          kokkos_impl_hip_shared_memory<void>(), m_shmem_begin, m_shmem_size,
          static_cast<void*>(
              static_cast<char*>(m_scratch_ptr[1]) +
              ptrdiff_t(threadid /
  ...
  ParallelFor(FunctorType const& arg_functor, Policy const& arg_policy)
      : m_functor(arg_functor),
        m_policy(arg_policy),
        m_league_size(m_policy.league_size()),
        m_league_sizes(m_policy.league_sizes()),
        m_team_size(m_policy.team_size()),
        m_vector_size(m_policy.impl_vector_length()) {
    auto internal_space_instance =
        m_policy.space().impl_internal_space_instance();
